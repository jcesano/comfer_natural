#save_res <- function(results, pars, n0, nsim, delete = FALSE){
save_res <- function(results, pars, res_path, seq, nsim, delete = FALSE){
  ...
  #dir_names <- rep(lapply(1:n0, p_set), each= nsim)
  dir_names <- rep(lapply(seq, p_set), each= nsim)
  
  #res_names <- rep(lapply(seq, res), nsim)
  #res_names <- lapply(1:(n0*nsim), res)
  res_names <- lapply(1:(seq_max*nsim), res)
